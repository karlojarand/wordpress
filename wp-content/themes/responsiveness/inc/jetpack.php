 ...
 */
function responsiveness_jetpack_setup() {
	// Add theme support for Infinite Scroll.
	add_theme_support( 'infinite-scroll', array(
		'footer_widgets'=> 'sidebar-1',
                'container'     => 'post-archives',
		'render'	=> 'responsiveness_infinite_scroll_render',
		'footer'	=> 'colophon',
                'post_per_page' => 8,
	) );
	...
	add_image_size( 'responsiveness-logo', 200, 200 );
	add_theme_support( 'site-logo', array( 'size' => 'responsiveness-logo' ) );
 ...
 */
function responsiveness_jetpack_default_hide_cf( $hidden, $screen ) {
...
add_action( 'default_hidden_meta_boxes', 'responsiveness_jetpack_default_hide_cf', 10, 2 );

/**
 * Custom render function for Infinite Scroll.
 */
function responsiveness_infinite_scroll_render() {
	while ( have_posts() ) {
		the_post();
		if ( is_search() ) :
 ...
 */
function responsiveness_the_site_logo() {
...
if( !function_exists( 'responsiveness_social_menu' ) ) :
 ...
 */
global $responsiveness_current_class;
$responsiveness_current_class = 'odd';
function responsiveness_odd_even_post_class( $classes ) {
    if( get_post_type( get_the_ID() ) === 'jetpack-testimonial' ) {
        global $responsiveness_current_class;
        $classes[] = $responsiveness_current_class;
        $responsiveness_current_class = ( $responsiveness_current_class == 'odd' ) ? 'even' : 'odd';
    }
    return $classes;
}
add_filter( 'post_class', 'responsiveness_odd_even_post_class' );

/**
 * Move JetPack Share and Like buttons
 * @link https://jetpack.com/2013/06/10/moving-sharing-icons/
 */
function responsiveness_jptweak_remove_share() {
...
add_action( 'loop_start', 'responsiveness_jptweak_remove_share' );