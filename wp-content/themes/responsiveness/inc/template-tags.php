...
if ( ! function_exists( 'responsiveness_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function responsiveness_posted_on() {
        global $post;
    
	$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '' .
                            esc_attr__( '', 'responsiveness' ) .
                            '' . 
                        '' . 
	...
	$byline = sprintf(
		...
		esc_html_x( 'By %s', 'post author', 'responsiveness' ),
	...
	//echo '<span class="byline"> ' . $byline . '</span>';
		...
		/* translators: used between list items, there is a space after the comma */
                if( 'post' === get_post_type() ) {
                    $categories_list = get_the_category_list( __( '</li><li>', 'responsiveness' ) );
...
if ( ! function_exists( 'responsiveness_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function responsiveness_entry_footer() {
    global $post;
	...
	if ( 'post' === get_post_type() || 'jetpack-portfolio' === get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
            if( 'post' === get_post_type() ) {
		$categories_list = get_the_category_list( esc_html__( ', ', 'responsiveness' ) );
            } elseif( 'jetpack-portfolio' === get_post_type() ) {
                $categories_list = get_the_term_list( $post->ID, 'jetpack-portfolio-type', '', esc_html_x(', ', 'Used between list items, there is a space after the comma.', 'responsiveness' ), '');
            }
		if ( $categories_list && responsiveness_categorized_blog() ) {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'responsiveness' ), esc_html__( '1 Comment', 'responsiveness' ), esc_html__( '% Comments', 'responsiveness' ) );
		echo '</span>';
	}
}
endif;
 ...
 * @return bool
 */
function responsiveness_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'responsiveness_categories' ) ) ) {
		...
		set_transient( 'responsiveness_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so responsiveness_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so responsiveness_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in responsiveness_categorized_blog.
 */
function responsiveness_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'responsiveness_categories' );
}
add_action( 'edit_category', 'responsiveness_category_transient_flusher' );
add_action( 'save_post',     'responsiveness_category_transient_flusher' );



/*==============================================================================
 * responsiveness CUSTOM TAGS BELOW
 =============================================================================*/
/**
 * Fancy excerpts
 * 
...
function responsiveness_fancy_excerpt() {
    ...
    if( is_archive() ) {
        ...
        echo '<a class="more-link more-link-activated" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'responsiveness' ) . get_the_title() . '" rel="bookmark">' . esc_html__( 'Continue Reading', 'responsiveness' ) . '</a>'; 
        echo '</div>';
    } elseif ( is_page_template( 'page-templates/page-child-pages.php' ) ) {
        the_excerpt();
         echo '<div class="continue-reading">';
        echo '<a class="continue-reading-arrow" href="' . esc_url( get_permalink() ) . '" title="' . esc_html__( 'Continue Reading ', 'responsiveness' ) . get_the_title() . '" rel="bookmark">&rarr;</a>'; 
        echo '</div>';
 ...
 */
function responsiveness_excerpt_more( $more ) {
    if( is_admin() ) {
        return $more;
    }
    return " &hellip;";
}
add_filter( 'excerpt_more', 'responsiveness_excerpt_more' );

/**
 * Add an author box below posts
 * @link http://www.wpbeginner.com/wp-tutorials/how-to-add-an-author-info-box-in-wordpress-posts/
 */
function responsiveness_author_box() {
    global $post;
    ...
    if ( isset( $post->post_author ) ) {
        ...
        $author_details .= '<h3 class="author-title"><span>' . esc_html__( 'About ', 'responsiveness' );
            if ( is_author() ) $author_details .= $display_name;        // If an author archive, just show the author name
            else $author_details .= esc_html__( 'the Author', 'responsiveness' ); // If a regular page, show "About the Author"
        ...
        if ( ! is_author() ) {  // Don't show the meta info on an author archive page
            $author_details .= '<p class="author-links entry-meta"><span class="vcard">' . esc_html__( 'All posts by', 'responsiveness' ) . '<a class="fn" href="' . esc_url( $user_posts ) . '">' . $display_name . '</a></span>';

            // Check if author has a website in their profile
            if ( ! empty( $user_site ) ) 
                $author_details .= '<a class="author-site" href="' . esc_url( $user_site ) . '" target="_blank" rel="nofollow">' . esc_html__( 'Website', 'responsiveness' ) . '</a></p>';
            else $author_details .= '</p>';
        }
        
        $author_details .= '</section>';
        $author_details .= '</div>';
        $author_details .= '<p class="show-hide-author label">' . esc_html__( 'Hide', 'responsiveness' ) . '</p>';
...
function responsiveness_portfolio_index_footer() {
    $time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time>';
    if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
            $time_string = '<i class="fa fa-refresh tip"><span class="tooltip">' .
                        esc_attr__( 'First posted: ', 'responsiveness' ) .
    ...
    $project_type = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta cat-links">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', 'responsiveness' ), '</span>' );
...
function responsiveness_breadcrumbs() {
    ...
    $home_link = '<a aria-label="' . esc_html__( 'Home', 'responsiveness' ) .'" title="' . esc_html__( 'Home', 'responsiveness' ) .'" class="breadcrumb-home" href="' . esc_url( home_url() ) . '"><i class="fa fa-home"></i></a>' . $separator;
    
    $output .= "<div aria-label='" . esc_html__( 'You are here:', 'responsiveness' ) . "' id='$breadcrumb_id' class='$breadcrumb_class'>";
    $output .= $home_link;
 ...
 * Social Menu
 */
function responsiveness_social_menu() {
 ...
 */
function responsiveness_post_icon() {
...
function responsiveness_the_post_icon() {
    echo wp_kses( responsiveness_post_icon(), array( 'span' => array( 'class' => array() ) ) );
}

/**
 * Function to show the Jetpack sharing and Likes only at the designated locations in Posts and Pages
 */
function responsiveness_jetpack_sharing() {
 ...
 */
function responsiveness_post_navigation() {
    ...
    <nav class="navigation post-navigation" role="navigation">
        ...
        <div class="nav-links" data-equalizer>
                <?php
                        previous_post_link( '<div class="nav-previous" data-equalizer-watch><div class="nav-indicator">' . esc_html_x( 'Previous Post:', 'Previous post', 'responsiveness' ) . '</div><h4>%link</h4></div>', '%title' );
                        next_post_link(     '<div class="nav-next" data-equalizer-watch><div class="nav-indicator">'     . esc_html_x( 'Next Post:', 'Next post', 'responsiveness' ) . '</div><h4>%link</h4></div>', '%title' );
                ?>
        </div> <!-- .nav-links -->
    </nav> <!-- .navigation -->
    <?php
...
if ( ! function_exists( 'responsiveness_paging_nav' ) ) :
/**
 ...
 */
function responsiveness_paging_nav() {
        the_posts_pagination( array(               
            'mid_size'      => 3,
            'prev_text' => __( 'Previous', 'responsiveness' ),
            'next_text' => __( 'Next', 'responsiveness' ),
            'type'      => 'list',
        ));
}
endif;
...
if ( ! function_exists( 'responsiveness_copyright' ) ) :
/** 
 ...
 */
function responsiveness_copyright() {
 ...
 */
function responsiveness_front_page_section( $partial = null, $page_name = "" ) {
	...
	if ( get_theme_mod( 'panel_' . $page_name ) ) {
		...
		echo '<article class="panel-placeholder panel responsiveness-panel responsiveness-panel-' . $page_name . '" id="panel-' . $page_name . '"><span class="responsiveness-panel-title">' . sprintf( __( '%1$s Page Placeholder', 'responsiveness' ), $page_name ) . '</span></article>';
	}
}